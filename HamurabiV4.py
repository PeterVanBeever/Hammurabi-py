...
class Hammurabi:
    ...
    def askHowManyAcresToPlant(self, acresOwned, population, bushels):
        ...
        try:
            while True:
                ...
                if harvest_acres <= acresOwned and (bushels/2) == harvest_acres and harvest_acres == (population * 10):
                    harvestedBushels = self.harvestRate * harvest_acres
                    bushels += harvestedBushels
                    self.storage_bushels = bushels
                    print(f"Harvested {harvestedBushels} at {self.harvestRate} per acre.")
                    return
                else: 
    ...
    def plagueDeaths(self):
        if self.rand.randint(0, 99) < 15:  # 15% chance of plague
    ...
    def grainEatenByRats(self, bushels):
        if self.rand.random() > self.rat_chance:  # 60% chance of no rat infestation
            self.bushel_storage_change = 0  # Ensure it is set to zero if no infestation
            self.rat_infestation = False
        else:
            bushel_adjust_percentage = self.rand.randint(10, 30)
    ...
    def printSummary(self):
            ...
            if self.starvationCounter > 0:
                ...
                print(f"In the previous year 0 people starved to death.")    
            ...
            if self.land_value_change:
                print(f"\n--------PRICE CHANGE NOTICE: {self.lastYearLandValue} now {self.land_value} ")
