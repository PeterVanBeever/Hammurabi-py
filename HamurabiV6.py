# Hammurabi Game Package
# Module
# Author: [Peter Van Beever]
# Version: 1.0.4
# Description: This package contains the Hammurabi game implementation.
import random
...
class Hammurabi:
    ...
    def game_over(self):
        ...
        print("----------GAME OVER----------\n\n")
        raise GameOverException("Game over due to excessive starvation.")
    ...
    def askHowManyAcresToPlant(self, acresOwned, population, bushels):
        ...
        try:
            while True:
                print(f"You have {acresOwned} acres, {bushels} bushels in storage, and {self.population} {population} people available.")
                plant_acres = int(input("How many acres would you like to plant? "))
                if plant_acres == 0:
                    return plant_acres
                
                # Check if the number of acres to plant is less than or equal to the acres owned
                if plant_acres > acresOwned:
                ...
                if plant_acres * self.farm_cost > bushels:
                ...
                if plant_acres > population * self.farm_acres_per_person:
                ...
                self.harvestedBushels = self.harvestRate * plant_acres
                self.storage_bushels += self.harvestedBushels - (plant_acres * self.farm_cost)
                print(f"\nHarvested {self.harvestedBushels} bushels at {self.harvestRate} bushels per acre.")
                return plant_acres
        except ValueError:
            print("Please enter a valid number.")
            self.sanity_counter += 1
        ...
        try:
            while True:
                ...
                if total_distributed_bushels > storage:
                    ...
                    print(f"\nYou served {total_distributed_bushels} bushels to feed {self.population}")
                    print(f"You now have {self.storage_bushels} bushels left in storage.")
                    # self.starvationCounter = self.starvationDeaths(total_distributed_bushels, people)
                return
        except ValueError:
            print("Please enter a valid number.")
            self.sanity_counter += 1
    ...
    def uprising(self, population, starved):
        return starved > (self.starvation_limit / 100) * population
    ...
    def printSummary(self):
            ...
            #print(f"In the previous year 5 people entered the kingdom.")
            ...
            print(f"The city owns 1000 acres of land.")
