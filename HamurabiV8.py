...
class Hammurabi:
    ...
    def playGame(self):
        ...
        while self.yearCounter <= 11:
            ...
            self.plagueDeaths(self.population)
            self.newCostOfLand()
        ...
        try:
            while True:
                ...
                if total_distributed_bushels > storage:
                    ...
                    # Check for uprising immediately after feeding
                    if self.uprising(self.population, self.starvationCounter):
                        print("----------GAME OVER----------")
                        print("The people have revolted due to excessive starvation.")
                        print("-----------------------------\n")
                        self.finalSummary()
                        exit()  # Exit the game loop immediately
                else:
                    self.storage_bushels -= total_distributed_bushels
    ...
    def harvest(self, acres):
        # Returns a random harvest between 1 and 6 bushels per acre
